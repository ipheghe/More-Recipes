...
const UserNavHeader = ({ firstName, lastName, onLogout }) => {
	return (
		<div>
			<header>
				<nav className="navbar navbar-toggleable-md fixed-top navbar-light bg-faded" id="brand-color">
					...
					<div className="collapse navbar-collapse" id="navbarSupportedContent" style={{ float: 'right' }}>
						...
						<ul className="nav justify-content-end" id="profile-nav">
							...
							<li className="nav-item">
								<a className="nav-link" href="./profile.html"><span><i className="fa fa-user" aria-hidden="true"></i></span> {firstName} {lastName}</a>
							</li>
							<li className="nav-item">
								<a className="nav-link" href=""><span><i className="fa fa-sign-out" aria-hidden="true" onClick={onLogout}></i></span> Logout</a>
							</li>
						</ul>
					</div>
				</nav>
